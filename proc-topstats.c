...
static int cpu_show(struct seq_file *p, void *v)
{
	int i;
	u64 used;
	u64 total;
...
static void show_val_kb(struct seq_file *m, char *spc, unsigned long num)
{
	seq_put_decimal_ull(m, spc, num << (PAGE_SHIFT - 10));
}

static int mem_show(struct seq_file *p, void *v)
{
	struct sysinfo i;
	long cached;
...
static int cpu_open(struct inode *inodep, struct file *filep)
{
	return single_open(filep, cpu_show, NULL);
}
...
static int mem_open(struct inode *inodep, struct file *filep)
{
	return single_open(filep, mem_show, NULL);
}
...
struct proc_dir_entry *topstats_dir;

static int __init me_init(void)
{
	printk(KERN_INFO "proc-topstats: init.\n");
	topstats_dir = proc_mkdir("topstats", NULL);
	proc_create("cpu", 0, topstats_dir, &cpu_fops);
	proc_create("mem", 0, topstats_dir, &mem_fops);
	return 0;
}

static void __exit me_exit(void)
{
	printk(KERN_INFO "proc-topstats: exit.\n");
	remove_proc_entry("cpu", topstats_dir);
	remove_proc_entry("mem", topstats_dir);
	remove_proc_entry("topstats", NULL);
}

MODULE_LICENSE("GPL");
MODULE_AUTHOR("Andre Osku Schmidt");
